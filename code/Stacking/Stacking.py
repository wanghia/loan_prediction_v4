...
class Ensemble(object):
	def __init__(self, n_folds,sampling, base_models):
	...
	def fit_predict(self, X, y, T):
		...
		pkl_filename  = "model/models_stacking.pkl"
		...
		for i, clf in enumerate(self.base_models):
			...
			for k in range(self.Sample_iter):
				...
				for j, (train_idx, test_idx) in enumerate(folds):
					...
					T_train[test_idx, k] = clf.predict_proba(X_holdout)[:,1]
					KFCV_test[:, j] = clf.predict_proba(T[:,feature_slice])[:,1]
			...
			if (i+1)%2==0:
				sio.savemat('model/Stacking_data_tree.mat', {'Train_data': A_train,'Train_Y':y,'Test_data':A_test})
		f.close()
		sio.savemat('model/Stacking_data_tree.mat', {'Train_data': A_train,'Train_Y':y,'Test_data':A_test})
